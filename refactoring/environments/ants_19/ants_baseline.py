...
def plot(ep, rewards_x_ep, actions_x_ep, ticks, actions_name):
    ...
    df.to_csv("environments/ants_15/metrics.csv", sep=',', index=False)
    ...
    fig.tight_layout()
    plt.savefig("environments/ants_15/Avg_Reward")
    ...
    plt.plot(x, y, label="mean", marker='x', markersize=.5, linewidth=.5, color='#DC267F')
    fig.tight_layout()
    plt.savefig("environments/ants_15/Avg_Ticks")
    ...
    plt.legend()
    plt.savefig("environments/ants_15/Actions")
