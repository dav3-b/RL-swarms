...
class Ants(AECEnv):
    ...
    def __init__(self, seed, render_mode: Optional[str] = None, **kwargs):
        ...
        self.ph_reward = kwargs['ph_reward']
    ...
    def process_agent(self, rewards_cust, action):
        """
        In this methods we compute the agent's reward and it's observation.
        """
        if self.obs_type == "paper":
            obs = self._get_obs2(self.learners[self.agent])
        elif self.obs_type == "variation1":
            obs = self._get_obs3(self.learners[self.agent])
    ...
    def step(self, action: int):
        ...
        self.observations[str(self.agent)], self.rewards_cust = self.process_agent(
            self.rewards_cust,
            action
        )
