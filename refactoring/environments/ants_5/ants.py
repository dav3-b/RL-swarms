...
class Ants(AECEnv):
    ...
    def __init__(self, seed, render_mode: Optional[str] = None, **kwargs):
        ...
        #self.food_pos_1 = (self.coords[0][0] + self.patch_size * int(self.W * 1/8), self.coords[0][1] + self.patch_size * int(self.H * 1/8)) 
        #self.food_pos_2 = (self.coords[0][0] + self.patch_size * int(self.W * 1/2), self.coords[0][1] + self.patch_size * int(self.H * 10/11)) 
        #self.food_pos_3 = (self.coords[0][0] + self.patch_size * int(self.W * 7/8), self.coords[0][1] + self.patch_size * int(self.H * 1/8)) 
    ...
    def lay_nest_pheromone(self, patches):
        ...
        for p in self.reward_patches[self.nest_pos]:
            nest_id = patches[p]['id']
            grid[nest_id] = 150

        #grid = gaussian_filter(grid.reshape((self.W, self.H)), sigma=7, radius=None, mode='wrap').flatten()
        grid = gaussian_filter(grid.reshape((self.W, self.H)), sigma=8, radius=None, mode='constant').flatten()
    ...
    def _check_pos(self, turtle):
        ...
        if turtle['pos'] in self.reward_patches[self.food_pos_1] and turtle['flags'] == [0, 0, 0]: 
            #if self.agent == 2:
            #    breakpoint()
            #breakpoint()
            #self.rewards_cust[self.agent].append(0.0)
            turtle['flags'][0] = 1
        elif turtle['pos'] in self.reward_patches[self.food_pos_2] and turtle['flags'] == [0, 0, 0]: 
            #if self.agent == 2:
            #    breakpoint()
            #breakpoint()
            #self.rewards_cust[self.agent].append(0.0)
            turtle['flags'][0] = 1
        elif turtle['pos'] in self.reward_patches[self.food_pos_3] and turtle['flags'] == [0, 0, 0]: 
            #if self.agent == 2:
            #    breakpoint()
            #breakpoint()
            #self.rewards_cust[self.agent].append(0.0)
            turtle['flags'][0] = 1
        elif turtle['pos'] in self.reward_patches[self.nest_pos] and turtle['flags'] == [1, 0, 1]: 
            #if self.agent == 2:
            #    breakpoint()
            #breakpoint()
            #self.rewards_cust[self.agent].append(0.0)
    ...
    def _check_food(self):
        if(
            len(self.reward_patches[self.food_pos_1]) == 0 and
            len(self.reward_patches[self.food_pos_2]) == 0 and
            len(self.reward_patches[self.food_pos_3]) == 0
        ):
            self.no_food = True
...
class AntsVisualizer:
    ...
    def render(
        ...
        for p in patches:
            ...
            if p in reward_patches[food_pos_1]:
                text = self.ph_pos_font.render('F', True, WHITE)
                self.screen.blit(text, text.get_rect(center=p))
            elif p in reward_patches[food_pos_2]:
                text = self.ph_pos_font.render('F', True, WHITE)
                self.screen.blit(text, text.get_rect(center=p))
            elif p in reward_patches[food_pos_3]:
                text = self.ph_pos_font.render('F', True, WHITE)
                self.screen.blit(text, text.get_rect(center=p))
            elif p in reward_patches[nest_pos]:
